...
class YahooStockWatch(object):
    ...
    def get_daily_stats(self,stock_id):
        ...
        for stock in stock_csvreader:
            return_data['open_num'] = stock[0]
            return_data['previous_close'] = stock[1]
            return_data['52week_high'] = stock[2].split('-')[0].strip()
            return_data['52week_low'] = stock[2].split('-')[1].strip()
            return_data['market_cap'] = stock[3]
        return return_data
